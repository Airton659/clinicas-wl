#!/bin/bash
#
# Script para criar perfis padrão usando Secret Manager
#
# Uso: ./seed_default_roles.sh <nome-cliente> <negocio-id>
# Exemplo: ./seed_default_roles.sh clinica-medica UtFHQf3lwIHfMmf8wHDu
#
...
if [ -z "$1" ] || [ -z "$2" ]; then
    ...
    echo ""
    echo "Uso: ./seed_default_roles.sh <nome-cliente> <negocio-id>"
    echo "Exemplo: ./seed_default_roles.sh clinica-medica UtFHQf3lwIHfMmf8wHDu"
    exit 1
fi
...
# Executar seed
echo "🚀 Executando seed de perfis..."
echo ""

python3 "$SCRIPT_DIR/seed_default_roles.py" "$CRED_FILE" "$NEGOCIO_ID"
